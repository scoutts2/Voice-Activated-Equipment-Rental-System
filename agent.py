...
3. Requirements Confirmation - Verify site safety and operator requirements
...
- ALWAYS start by calling get_current_stage_tool() to check your stage
- Use get_stage_instructions_tool() to get detailed instructions for the current stage
- Follow the stage instructions exactly - they tell you which tools to call and when
- Call move_to_next_stage_tool() when you complete each stage
- Be friendly and professional throughout the conversation
- Confirm details by repeating them back to the customer

Start by greeting the customer and calling get_current_stage_tool() to begin the process.
"""
    
    logger.info(f"Loaded {len(available_equipment)} available equipment items")
    
    # Create the voice agent with AI models
    agent = Agent(
        instructions=initial_prompt,
        stt=livekit.plugins.deepgram.STT(),     # Speech-to-Text - converts voice to text
        llm=livekit.plugins.openai.LLM(model="gpt-4o"),  # Large Language Model - the AI brain
        tts=livekit.plugins.elevenlabs.TTS(),   # Text-to-Speech - converts AI response to voice
    )
    
    logger.info("Voice agent created and configured")
    
    # Start the agent session
    session = agent.session(ctx.room)
    await session.start()
    
    logger.info("Voice agent started, ready to converse")
...
@llm.function_tool()
def verify_business_license_tool(license_number: str):
...
@llm.function_tool()
def get_equipment_details_tool(equipment_id: str):
...
@llm.function_tool()
def book_equipment_tool(equipment_id: str):
...
@llm.function_tool()
def get_current_stage_tool() -> str:
...
Stage 3: Requirements Confirmation - Verify site safety & operator needs
...
@llm.function_tool()
def move_to_next_stage_tool() -> str:
...
@llm.function_tool()
def verify_operator_credentials_tool(operator_license: str, certification_type: str) -> str:
...
@llm.function_tool()
def verify_site_safety_tool(job_address: str, equipment_category: str, weight_class: str) -> str:
...
@llm.function_tool()
def verify_insurance_coverage_tool(policy_number: str, required_amount: int, equipment_value: int) -> str:
...
@llm.function_tool()
def get_stage_instructions_tool() -> str:
...
- When they've selected equipment, call move_to_next_stage_tool()""",
        ...
        3: """Stage 3 - Requirements Confirmation:
...
- Ask about operator certification needs
- Verify they have the required certifications
- When requirements are confirmed, call move_to_next_stage_tool()""",
...
- Call book_equipment_tool(equipment_id) to finalize the rental
- Confirm all details (equipment, price, dates, operator, insurance)
- Provide pickup location and contact information
